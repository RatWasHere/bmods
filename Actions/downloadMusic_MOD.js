  ...
  async run(values, message, client, bridge) {
    ...
    if (platform == "win32") {
      ...
      const missingFiles = dependencies.filter(
        (file) => existsSync(path.join(projectPath, file)) == false,
      );
    ...
    await new Promise((res, rej) => {
      ...
      if (
        bridge.transf(values.useCustomName) == "Yes" &&
        bridge.transf(values.customName).length > 0
      ) {
        command = command.replace(
          "%(title)s",
          bridge
            .transf(values.customName)
            .replaceAll("|", "")
            .replaceAll(" ", "_"),
        );
      }
      ...
      if (
        bridge.transf(values.useBrowserCookies) == "Yes" &&
        bridge.transf(values.browserCookiesInfo).length > 0
      ) {
        command = command.replace(
          "BROWSER[+KEYRING][:PROFILE][::CONTAINER]",
          bridge.transf(values.browserCookiesInfo).replaceAll("|", ""),
        );
      } else {
        command = command.replace(
      ...
      if (
        bridge.transf(values.useArguments) == "Yes" &&
        bridge.transf(values.arguments).length > 0
      ) {
        command = command.replace(
          "<additionalArgs>",
          bridge.transf(values.arguments).replaceAll("|", ""),
        );
      } else {
      ...
      require("child_process").exec(fcommand, (error, stdout) => {
        ...
        if (
          stdout.includes("[ExtractAudio] Destination:") &&
          stdout.includes("[download] 100%")
        ) {
          ...
          if (match) {
            ...
            if (values.delete == true) {
              let deleteCommand;
              if (platform === "win32")
                deleteCommand = `del /f /q "${fileSource}"`;
              else if (platform === "linux")
                deleteCommand = `rm -rf "${fileSource}"`;
          ...
          stdout.includes("[ExtractAudio] Not converting audio")
        ) {
