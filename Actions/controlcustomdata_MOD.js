module.exports = {
            ...
            UI: [
              {
                ...
                UItypes: {
                  data: {
                    name: "Create Data",
                    preview: "`Query: ${option.data.path} - ${option.data.value}`",
                    data: { query: "" , value: "" },
                    UI: [
                      {
                        element: "input",
    ...
    run(values, message, client, bridge) {
      ...
      for (const dataCase of values.cases) {
        ...
        file.set(bridge.transf(dataCase.data.path), bridge.transf(dataCase.data.value));
      };
      for (const dataCase of values.cases3) {
        if (dataCase.type !== "data") continue;
        file.append(bridge.transf(dataCase.data.path), bridge.transf(dataCase.data.value));
      };
