...
module.exports = {
    ...
    "-",
    {
      ...
      UItypes: {
        servers: {
          ...
          name: "Server:",
          preview:
            "`${option.data.ipAddress}:${option.data.ipPort} | Command: ${option.data.rconCommand}`",
          UI: [
            {
              element: "input",
              storeAs: "ipAddress",
  ...
  async run(values, interaction, client, bridge) {
    ...
    for (let server of values.serverList) {
      const timeout = bridge.transf(server.data.timeout)
        ? Number(bridge.transf(server.data.timeout)) * 1000
        : 5000;
            ...
            const rconServer = rcon
              .connect({
                onSuccess: () => {
                  if (logging == true) {
                    console.log(
                      `Connection to ${ipAddr}:${ipPort} established.`
                    );
                  }
                },
                onError: (error) => {
